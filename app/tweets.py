...
def twt__1(company_name: str, job_title: str, limit: int = 5):
    """
    ...
    query_params = {
        'type': 'Top',
        'count': limit,
        'query': f'("{company_name}" OR "{company_name} inc") OR ("ceo of {company_name}" OR "{company_name} ceo") OR ("{job_title}" AND ("{company_name}" OR "{company_name} inc")) -filter:retweets lang:en',
    }
...
def twt__2(company_name: str, job_title: str, limit: int = 5):
...
def twt__3(company_name: str, limit: int = 5):
    try:
        ...
        url_safe_query = urllib.parse.urlencode(query_params)
        main_filter = "%28%22apple%22+OR+%22apple+inc%22%29+OR+%28%22ceo+of+apple%22+OR+%22apple+ceo%22%29+OR+%28%22software+engineer%22+AND+%28%22apple%22+OR+%22apple+inc%22%29%29+-filter%3Aretweets+lang%3Aen"
        conn.request("GET", f"/search/={main_filter}?{url_safe_query}", headers=headers)
        res = conn.getresponse()
        data = res.read()
        output_dict = json.loads(data)
...
# Threshold: 500 Requests / MO
def twt__4(company_name: str, job_title: str, limit: int = 5):
    try:
...
# Threshold: 100 / MO
def twt__5(company_name: str, job_title: str, limit: int = 5):
    try:
