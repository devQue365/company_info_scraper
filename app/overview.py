...
def ov__1(company_name : str, db : Session):
    try:
        # check if the data exists in the database and is fresh right now
        try:
            # always reset the table first
            reset_table(CACHE_OVR, db, _member = "company_name")
            # niw consider fetching overview
            overview = db.query(CACHE_OVR).filter(CACHE_OVR.company_name == company_name.lower()).first().company_overview
            print("\033[35m\033[1mGot the overview from database\033[0m")
            return overview # directly return the overview
        except:
            pass
        query_params = {
            "query": company_name,
        ...
        def summarize(data):
            ...
            return container
        overview = summarize(response.json())
        # add the details to database collection
        new_entry = CACHE_OVR(
